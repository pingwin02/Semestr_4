...
class FrozenLake(Env):
    def __init__(self, name='FrozenLake'):
        ...
        self.action_prob = {
            0: np.array([self.intended_action_prob, self.opposite_from_intended_prob, self.side_from_intended_prob,
                         self.side_from_intended_prob]),
            1: np.array([self.opposite_from_intended_prob, self.intended_action_prob, self.side_from_intended_prob,
                         self.side_from_intended_prob]),
            2: np.array([self.side_from_intended_prob, self.side_from_intended_prob, self.intended_action_prob,
                         self.opposite_from_intended_prob]),
            3: np.array([self.side_from_intended_prob, self.side_from_intended_prob, self.opposite_from_intended_prob,
        ...
        for y in range(len(self.grid_world)):
            for x in range(len(self.grid_world[y])):
                ...
                if q_values is not None and (self.grid_world[y][x] == self.regular_field):
                    ...
                    if q_values[state, sorted_actions[3]] > q_values[
                        state, sorted_actions[2]]:  # there is a clear best action
                ...
                if q_values is not None and self.grid_world[y][x] == self.regular_field:
                    ...
                    for a in range(4):
                        ...
                        msg = self.assets['q_values_font'].render(f"{q_values[self.get_state(y, x), a]:04.1f}", False,
                                                                  color)
